...
async function ensureAdmin() {
  ...
  if (
    ...
    console.log(
      "Admin user already exists. Updating role to admin and confirming email if needed..."
    );
    // Find the user by email
    const existing = await findUserByEmail(admin, adminEmail);
    if (!existing) {
      console.error(
        "User exists but could not be found via listUsers(). Check Supabase auth users."
      );
      process.exit(1);
    }

...
export function MemberManagement({ members, onMembersChange }: { members: Member[]; onMembersChange: () => void }) {
  ...
  const handleRemove = async (id: string, name: string) => {
    ...
    try {
      ...
      const { error: shiftsError } = await supabase.from("shifts").delete().eq("member_id", id)
      if (shiftsError) {
    ...
    return (
      <Card>
        <CardContent className="py-8">
          <p className="text-center text-gray-500 text-sm">Please log in to manage members.</p>
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setNewMember((p) => ({ ...p, employeeId: e.target.value }))}
                required
                disabled={loading}
                className="text-sm"
              />
        ...
        <div className="space-y-2">
          <h4 className="font-medium text-gray-900 text-sm">Current Members ({members.length})</h4>
          <div className="max-h-48 sm:max-h-60 overflow-y-auto space-y-2">
            {members.map((m) => (
              <div key={m.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-sm truncate">{m.name}</div>
                  <div className="text-xs text-gray-500">ID: {m.employee_id}</div>
                </div>
                <Button
                  variant="ghost"
                  size="sm"

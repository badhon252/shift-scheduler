...
async function ensureAdminUser(email: string, password: string, fullName: string) {
  ok(!!url && !!serviceRole, "Supabase URL or Service Role key missing on server")
  ...
  if (!existing) throw new Error(`User not found via listUsers(). Check Supabase Auth for ${email}.`)
...
export async function GET(request: Request) {
  try {
    ...
    const token = searchParams.get("token") || request.headers.get("x-setup-token") || undefined
    if (!isAuthorizedToken(token || undefined)) {
      return NextResponse.json({ ok: false, error: "Unauthorized (bad token)" }, { status: 401 })
    }
    const res = await ensureAdminUser(DEFAULT_EMAIL, DEFAULT_PASSWORD, DEFAULT_NAME)
...
export async function POST(request: Request) {
  try {
    ...
    const token =
      request.headers.get("x-setup-token") || body.token || new URL(request.url).searchParams.get("token") || undefined
    if (!isAuthorizedToken(token || undefined)) {
      return NextResponse.json({ ok: false, error: "Unauthorized (bad token)" }, { status: 401 })
    }
